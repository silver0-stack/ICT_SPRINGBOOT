...
//테이블 생성에 대한 가이드 클래스임
//@Entity 어노테이션 반드시 표기함 => 설정정보에 자동 등록됨 => Repository 와 연결됨
...
@NoArgsConstructor
@Builder
@Table(name = "NOTICE") //매핑할 테이블 이름 지정함, NOTICE 테이블을 자동으로 만들어주기도 하는 어노테이션임
@Entity // JPA 가 관리함, DB 테이블과 DTO 클래스 매핑을 위해서 필요함
public class NoticeEntity {
    @Id     // JPA 가 Entity 객체들을 관리할 때 식별할 아이디 지정 용도의 어노테이션임
    ...
    @Column(name = "IMPORTANCE", nullable = false, columnDefinition = "CHAR(1) DEFAULT 'N'")
    ...
    @Column(name = "READCOUNT", nullable = false, columnDefinition = "NUMBER DEFAULT 1")
